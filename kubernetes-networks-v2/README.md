### C. kubernetes-networks-v2

- `minikube delete && minikube start`
- `cd kubernetes-networks-v2`
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ kubectl get gatewayclass
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ helm repo add traefik https://traefik.github.io/charts && helm repo update && \
helm install traefik traefik/traefik --namespace traefik --create-namespace -f ./additional/helm/traefic-values.yaml
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ kubectl apply -f gateway.yaml 
gateway.gateway.networking.k8s.io/homework-gateway created
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ kubectl get gateway
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ kubectl apply -f service.yaml 
service/basov-service created
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ kubectl get svc
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ kubectl apply -f httpRoute.yaml 
httproute.gateway.networking.k8s.io/homework-route created
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-networks-v2$ kubectl get httproute
NAME             HOSTNAMES           AGE
