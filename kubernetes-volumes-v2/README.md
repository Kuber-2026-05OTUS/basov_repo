### D. kubernetes-volumes-v2

- `minikube delete && minikube start`
- `cd kubernetes-volumes-v2`
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$ kubectl exec -it basov-deployment-6f9d677887-fvppv -- sh -c "wget -qO- http://127.0.0.1:8000/conf/basov-default.conf"
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$ kubectl get pods
NAME                               READY   STATUS    RESTARTS   AGE
...
```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$ kubectl exec -it basov-deployment-74789cf9f-kmldh -- sh -c "cat /homework/test-pvc"
Defaulted container "nginx-fork" out of: nginx-fork, wget-index-html (init)
The new episode 8 of From airs on June 14, 2026.
```

```bash
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$ kubectl delete deployment basov-deployment
deployment.apps "basov-deployment" deleted from homework namespace
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$ kubectl get deployments
No resources found in homework namespace.
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$ kubectl get pvc
NAME           STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS        VOLUMEATTRIBUTESCLASS   AGE
homework-pvc   Bound    pvc-b5b9fadd-9cfe-416f-bb9a-2d0381f685b6   1Gi        RWO            homework-hostpath   <unset>                 3m53s
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$ kubectl apply -f deployment.yaml 
...
ubuntu@ubuntu-YO.HONOR:~/otus/basov_repo/kubernetes-volumes-v2$  kubectl exec -it basov-deployment-74789cf9f-62xzr -- sh -c "cat /homework/test-pvc"
Defaulted container "nginx-fork" out of: nginx-fork, wget-index-html (init)
The new episode 8 of From airs on June 14, 2026.
```
